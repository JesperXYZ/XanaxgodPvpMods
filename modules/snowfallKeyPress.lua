...
function Module:GetOptions(myOptionsTable, db)
    ...
    local SnowfallKeyPressImage =
    "|TInterface\\Addons\\XanaxgodPvpMods\\media\\moduleImages\\SnowfallKeyPress:102:232:132:-30|t"
...
function Module:SetupUI()
    ...
    for i = 1, animationsCount do
        ...
        scale1:SetScale(1.0 * snowfallKeyPressSize, 1.0 * snowfallKeyPressSize)
        scale1:SetDuration(0)
        scale1:SetOrder(1)
        scale2 = animationGroup:CreateAnimation("Scale")
        scale2:SetScale(1.5 * snowfallKeyPressSize, 1.5 * snowfallKeyPressSize)
    ...
    if snowfallKeyPressSize > 0 and snowfallKeyPressGirth > 0 then
        ...
        self:SecureHook("MultiActionButtonDown", function(bname, id) AnimateButton(_G[bname .. "Button" .. id]) end)
    end
end
